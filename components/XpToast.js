      ...
      <div className="flex flex-col gap-2 min-w-[220px]">
        ...
        {event.xp > 0 && (
          <div className="flex items-center gap-3 glass border border-yellow-500/30 rounded-xl px-4 py-3 shadow-xl">
            ...
            <div>
              <p className="text-sm font-bold text-yellow-400">+{event.xp} XP</p>
        ...
        {event.leveledUp && (
          <div className="flex items-center gap-3 glass border border-blue-500/30 rounded-xl px-4 py-3 shadow-xl">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">You reached level {event.newLevel}</p>
            </div>
          </div>
        )}
        ...
        {event.newBadges?.map(badgeId => (
          <div key={badgeId} className="flex items-center gap-3 glass border border-purple-500/30 rounded-xl px-4 py-3 shadow-xl">
            <div className="w-8 h-8 bg-purple-500/20 rounded-full flex items-center justify-center">
              <Award className="w-4 h-4 text-purple-400" />
            </div>
            <div>
              <p className="text-sm font-bold text-purple-400">Badge Earned!</p>
              <p className="text-xs text-gray-400">{badgeId.replace(/_/g, " ")}</p>
            </div>
          </div>
        ))}
      </div>
