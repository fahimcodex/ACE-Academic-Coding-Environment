...
import {
  Brain, Map, Zap, Loader2, RefreshCw,
  Code2, ChevronDown, Sparkles,
} from "lucide-react";
...
const TOPICS = {
  python: ["Variables & Types", "Loops", "Functions", "Lists", "Strings", "Dictionaries", "Recursion", "File I/O"],
  c:      ["Variables", "Pointers", "Arrays", "Functions", "Structs", "Memory Management", "File I/O"],
  cpp:    ["Classes", "Inheritance", "Templates", "STL", "Pointers", "Vectors", "Strings"],
  linux:  ["Navigation", "File Management", "Permissions", "Grep & Search", "Pipes", "Shell Scripts"],
};
...
export default function AiPage() {
  ...
  const [practiceTopic,   setPracticeTopic]   = useState("Loops");
  ...
  useEffect(() => {
    setPracticeTopic(TOPICS[practiceLang]?.[0] ?? "");
  }, [practiceLang]);
  ...
  async function getPath() {
    ...
    try {
      ...
      const result = await getLearningPath(
        completedIds, profile?.xp ?? 0, profile?.level ?? 1, areas, user.uid
      );
  ...
  async function getPractice() {
    ...
    try {
      const result = await generatePractice(
        practiceTopic, practiceLang, practiceDiff, completedIds, user.uid
      );
      ...
      <div className="max-w-5xl mx-auto px-4 pt-28 pb-16">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-400 max-w-lg mx-auto">
            Get a personalized study plan and AI-generated practice problems — powered by Google Gemini, completely free.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="glass rounded-2xl p-6 border border-white/10">
              <p className="text-sm text-gray-400 mb-4">
                Based on your{" "}
                <span className="text-yellow-400 font-semibold">{profile?.xp ?? 0} XP</span>,{" "}
                <span className="text-blue-400 font-semibold">{completedIds.length} lessons</span> completed,
                and the topics you find difficult.
              </p>
              ...
              <button onClick={getPath} disabled={pathLoading}
            ...
            {pathError && (
              <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4">
                <p className="text-red-400 text-sm whitespace-pre-wrap">❌ {pathError}</p>
              </div>
            )}
            ...
            {pathResult && (
              <div className="glass rounded-2xl p-6 border border-blue-500/20">
                ...
                <div className="prose prose-invert prose-sm max-w-none">
                  <ReactMarkdown remarkPlugins={[remarkGfm]}>{pathResult}</ReactMarkdown>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="glass rounded-2xl p-6 border border-white/10">
              <p className="text-sm text-gray-400 mb-4">
                Generate a fresh coding problem targeted at exactly what you need to practice.
              </p>

              <div className="grid grid-cols-3 gap-3 mb-4">
                <Select value={practiceLang}  onChange={setPracticeLang}  options={LANGUAGES}                  label="Language"   />
                <Select value={practiceDiff}  onChange={setPracticeDiff}  options={DIFFICULTIES}               label="Difficulty" />
                <Select value={practiceTopic} onChange={setPracticeTopic} options={TOPICS[practiceLang] ?? []} label="Topic"      />
              </div>
            ...
            {practiceError && (
              <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4">
                <p className="text-red-400 text-sm whitespace-pre-wrap">❌ {practiceError}</p>
              </div>
            )}
                  ...
                  <div>
                    <div className="px-4 py-2 bg-white/[0.02] border-b border-white/10 flex items-center justify-between">
                      <span className="text-xs text-gray-400 font-mono">
                        {practiceLang === "cpp" ? "solution.cpp" : practiceLang === "c" ? "solution.c" : "solution.py"}
                      </span>
                      <span className="text-xs text-gray-500">Edit your solution here</span>
                    </div>
                    <MonacoEditor
                      height="220px"
                      language={practiceLang === "cpp" ? "cpp" : practiceLang === "c" ? "c" : "python"}
                      theme="vs-dark"
                      value={practiceCode}
                    ...
                    <div className="px-4 py-3 bg-white/[0.02] border-t border-white/10 text-center">
                      <p className="text-xs text-gray-500">
                        ...
                        <a href="/courses/python" className="text-blue-400 hover:underline">lesson editor</a>.
                      </p>
                    </div>
                  </div>
                )}
