...
import {
  Zap, Flame, Trophy, BookOpen, ChevronRight,
  Target, Calendar, Star, TrendingUp,
} from "lucide-react";
import { todayString } from "@/lib/gamification";

const COURSES = [
  { id:"python", emoji:"🐍", name:"Python",  color:"text-green-400",  border:"border-green-500/30",  bg:"hover:bg-green-500/5"  },
  { id:"linux",  emoji:"🐧", name:"Linux",   color:"text-orange-400", border:"border-orange-500/30", bg:"hover:bg-orange-500/5" },
  { id:"c",      emoji:"⚙️", name:"C",       color:"text-blue-400",   border:"border-blue-500/30",   bg:"hover:bg-blue-500/5"   },
  { id:"cpp",    emoji:"🔷", name:"C++",     color:"text-purple-400", border:"border-purple-500/30", bg:"hover:bg-purple-500/5" },
];
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (chalSnap.exists()) setTodayChallenge({ id: chalSnap.id, ...chalSnap.data() });
  ...
  COURSES.forEach(c => {
    completedByCourse[c.id] = progress.filter(p => p.courseId === c.id).length;
  });
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <div className="max-w-6xl mx-auto px-4 pt-28 pb-16">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400 mt-1">Keep up your momentum — you're doing great!</p>
        </div>

        {/* ── Top stats row ──────────────────────────────────────────────── */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { icon: Zap,      color: "text-yellow-400 bg-yellow-400/10", label: "Total XP",        value: totalXp.toLocaleString() },
            { icon: TrendingUp,color: "text-blue-400   bg-blue-400/10",  label: "Level",            value: level },
            { icon: Flame,    color: "text-orange-400  bg-orange-400/10", label: "Day Streak",      value: streak },
            { icon: Star,     color: "text-purple-400  bg-purple-400/10", label: "Badges",          value: badges.length },
          ...
          {todayChallenge ? (
            <Link href="/challenges"
              className={`glass rounded-2xl p-6 border flex items-center justify-between group transition-all card-hover block ${
                challengeDone ? "border-green-500/30" : "border-blue-500/20 hover:border-blue-500/40"
              }`}>
              <div>
                <div className="flex items-center gap-2 mb-1">
                  {challengeDone
                    ? <span className="text-xs text-green-400 font-semibold bg-green-500/10 px-2 py-0.5 rounded-full">✓ Completed</span>
                    : <span className="text-xs text-blue-400 font-semibold bg-blue-500/10 px-2 py-0.5 rounded-full">Today</span>
                  }
                ...
                <p className="text-gray-400 text-sm mt-1 line-clamp-1">{todayChallenge.description}</p>
              </div>
              <div className="flex items-center gap-3 ml-4">
                <div className="text-right">
                  <p className="text-yellow-400 font-bold flex items-center gap-1">
                    <Zap className="w-4 h-4" />{todayChallenge.xpReward} XP
                  </p>
                </div>
            ...
            <div className="glass rounded-2xl p-6 border border-white/5 text-gray-400 text-sm">
              No challenge today. Run <code className="text-blue-400">node scripts/seedChallenges.mjs</code> to add challenges.
            </div>
          )}
        </div>
              ...
              return (
                <Link key={course.id} href={`/courses/${course.id}`}
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`text-xs font-bold ${course.color}`}>{pct}%</span>
                  </div>
                  <p className="font-semibold">{course.name}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{done} lessons done</p>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <ChevronRight className={`w-4 h-4 ${course.color} group-hover:translate-x-1 transition-transform`} />
                  </div>
                </Link>
              );
            })}
          ...
          <div className="glass rounded-2xl border border-white/5 divide-y divide-white/5">
            {dataLoading ? (
              ...
              <div className="p-6 text-center text-gray-400 text-sm">
                ...
                <Link href="/courses" className="text-blue-400 hover:underline">Start your first lesson!</Link>
              </div>
            ) : (
              [...progress]
                  ...
                  <div key={i} className="flex items-center justify-between px-5 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">{item.courseId} · {item.lessonId}</p>
                      </div>
                    </div>
