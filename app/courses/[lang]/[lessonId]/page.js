  ...
  async function submitQuiz() {
    ...
    if (allCorrect && user) {
      ...
      if (updateProfile && profile) {
        updateProfile({ xp: expectedTotalXp });
      }
      ...
      if (result) {
        // Correct optimistic update with confirmed data from server
        if (updateProfile && profile) {
          updateProfile({
            xp: result.newXp ?? expectedTotalXp,
            level: result.newLevel ?? profile.level,
            streak: result.newStreak ?? profile.streak,
            badges: result.newBadges
              ? [...profile.badges, ...result.newBadges]
              : profile.badges,
          });
        }
