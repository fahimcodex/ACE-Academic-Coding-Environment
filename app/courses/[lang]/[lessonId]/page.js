...
import {
  ...
  ChevronRight, Clock, Terminal, Code2, BookOpen,
} from "lucide-react";
...
export default function LessonPage() {
    ...
    if (allCorrect && user) {
      ...
      if (result) {
        ...
        for (const badgeId of (result.newBadges ?? [])) {
          await notify.badge(user.uid, badgeId.replace(/_/g, " "));
        }
        if (result.newStreak === 3 || result.newStreak === 7 || result.newStreak === 30) {
  ...
  if (!lesson) return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center text-gray-400">Lesson not found.</div>
  );
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col">
      ...
      <div className="pt-16 border-b border-white/5 bg-gray-950/80 backdrop-blur sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
          <Link href={`/courses/${lang}`} className="flex items-center gap-2 text-gray-400 hover:text-white text-sm transition-colors">
            ...
            {isLinux ? (
              <div>
                <p className="text-sm text-gray-400 mb-3">
                  Practice Linux commands below. Type <code className="text-blue-400">help</code> to see all commands.
                </p>
                <LinuxTerminal />
              </div>
            ) : (
                ...
                <div className="glass rounded-xl overflow-hidden border border-white/10">
                  <div className="flex items-center justify-between px-4 py-2 bg-white/[0.02] border-b border-white/10">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-gray-500 hidden sm:block">Ctrl+Enter to run</span>
                      <button onClick={() => setCode(lesson.starterCode)}
                        className="flex items-center gap-1 text-xs text-gray-400 hover:text-white px-2 py-1 rounded hover:bg-white/5">
                        <RotateCcw className="w-3 h-3" /> Reset
                      </button>
                      <button onClick={runCode} disabled={running}
                        className="flex items-center gap-1.5 text-xs bg-green-600 hover:bg-green-500 disabled:opacity-50 text-white px-3 py-1.5 rounded font-semibold transition-colors">
                        <Play className="w-3 h-3" />{running ? "Running..." : "Run"}
                      </button>
                    </div>
                  </div>
                  <MonacoEditor height="420px" language={MONACO_LANG[lesson.language]} theme="vs-dark"
                    value={code} onChange={v => setCode(v ?? "")}
                    options={{ fontSize:14, minimap:{enabled:false}, scrollBeyondLastLine:false, padding:{top:12,bottom:12}, tabSize: lesson.language==="python"?4:2, wordWrap:"on" }} />
                </div>
                {/* Output */}
                <div className="glass rounded-xl overflow-hidden border border-white/10">
                  <div className="flex items-center justify-between px-4 py-2 bg-white/[0.02] border-b border-white/10">
                    <span className="text-xs text-gray-400">Output</span>
                    {execMeta && <span className="text-xs text-gray-500">{execMeta.time}s · {Math.round((execMeta.memory??0)/1024)}KB</span>}
                  </div>
                  <pre className={`p-4 text-sm font-mono whitespace-pre-wrap overflow-auto h-[420px] ${output.startsWith("❌")?"text-red-400":"text-green-400"}`}>
        ...
        {tab === "quiz" && (
          <div className="max-w-2xl mx-auto">
            ...
            <p className="text-gray-400 mb-8">
              Answer all correctly to earn <span className="text-yellow-400 font-semibold">+{lesson.xpReward} XP</span>.
            </p>
                ...
                <div key={q.id} className="glass rounded-xl p-6 border border-white/10">
                  ...
                  <div className="space-y-2">
                    {q.options.map((opt, oi) => {
                      ...
                      let cls = "border-white/10 hover:border-blue-500/40 bg-white/[0.02] hover:bg-blue-500/5 cursor-pointer";
                      if (submitted) {
