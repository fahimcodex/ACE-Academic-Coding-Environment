    ...
    try {
      if (lesson.language === "python") {
        ...
        if (result.error) setOutput(result.error);
          ...
          if (result.time)
            setExecMeta({ time: result.time, memory: result.memory });
        }
      } else {
    ...
    if (allCorrect && user) {
      ...
      if (result) {
        ...
        for (const b of result.newBadges ?? [])
          await notify.badge(user.uid, b.replace(/_/g, " "));
            ...
            {isLinux ? (
              <div>
                <p className="text-sm text-gray-400 mb-3">
                  Type <code className="text-blue-400">help</code> to see all
                  available commands.
                </p>
                <LinuxTerminal />
              </div>
              ...
              <div className="flex gap-4">
                {/* Editor + output */}
                <div
                  className={`flex flex-col gap-4 transition-all duration-300 ${aiPanel !== "none" ? "flex-1 min-w-0" : "w-full"}`}
                >
            ...
            {missionPhase === "intro" && (
              <div className="glass rounded-xl p-6 border border-white/10 text-center animate-in fade-in zoom-in duration-300">
                ...
                <p className="text-gray-400 mb-6">
                  Review your objectives before beginning.
                </p>
              ...
              <div className="animate-in slide-in-from-right duration-300">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-gray-400">
                    {lesson.quiz.length} objectives
                  </span>
                </div>
                <p className="text-gray-400 mb-8">
                  Complete all tasks perfectly to earn{" "}
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        {q.options.map((opt, oi) => {
                          ...
                          let cls =
                            "border-white/10 hover:border-blue-500/40 bg-white/[0.02] cursor-pointer";
                          if (submitted) {
                            if (isCorrect)
                              cls =
            ...
            {missionPhase === "completed" && (
              <div className="animate-in slide-in-from-bottom duration-300 mt-8 glass rounded-xl p-6 border border-white/10 text-center">
                ...
                <p className="text-gray-300 mb-8 text-lg">
                  {quizScore === lesson.quiz.length
                    ? "Data verified. Authorization granted. 🎉"
                    : "Mission failed. Review logic and retry."}
                </p>
